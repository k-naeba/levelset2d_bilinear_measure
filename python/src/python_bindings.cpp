...
// list of them), plus levelset2d_bilinear_measure's own functions.
// Deliberately does NOT depend on (or bind) any extraction-algorithm
// project (levelset2d_polygon) -- matching this project's own C++
// design, which only depends on common_geometry and operates on
...
PYBIND11_MODULE(_levelset2d_bilinear_measure, m) {
  ...
  for (const auto& o : ns_ls2bm::kCellCornerOffset)
    corner_offsets.append(py::make_tuple(o[0], o[1]));
  m.attr("CELL_CORNER_OFFSET") = corner_offsets;

  m.def("bilinear_value", &ns_ls2bm::BilinearValue, py::arg("v"), py::arg("x"), py::arg("y"),
        "v: the 4 cell-corner values, ordered per CELL_CORNER_OFFSET.");

  m.def("find_bilinear_crossings", &ns_ls2bm::FindBilinearCrossings, py::arg("v"),
  ...
  m.def("ray_segment_intersect", &ns_ls2bm::RaySegmentIntersect, py::arg("origin"),
        py::arg("dir"), py::arg("a"), py::arg("b"),
        "The ray parameter t where segment [origin, origin+dir] crosses "
        "segment (a,b), or None.");

  m.def("find_polygon_crossings", &ns_ls2bm::FindPolygonCrossings, py::arg("segments"),
